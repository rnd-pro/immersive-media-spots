 ...
 * & import('../wgt/audio/ImsAudioData.js').ImsAudioData
 * & import('../wgt/map/ImsMapData.js').ImsMapData>} ImsData
 */
...
export class ImsBaseClass extends Symbiote {
  ...
  constructor() {
    ...
    this.add$({
      ...
      ready: false,
      onFs: () => {
        let fsTarget = this.closest('ims-viewer') || this;
        this.$[imsCtxName + '/fsTarget'] = fsTarget;
  ...
  fillSrcVariantList() {
    if (this.srcData?.urlTemplate && this.srcData?.cdnIdList?.length) {
      ...
      this.srcData.srcList = this.srcData.cdnIdList.map((uid) => {
        return this.#makeUrl(uid, variant);
      });
    }
  }
  ...
  initCallback() {
    ...
    if (!FullscreenMgr.supported()) {
      let fsTarget = /** @type {HTMLElement} */ (this.closest('ims-viewer') || this);
      if (FullscreenMgr.cssFallbackBlocked(fsTarget)) {
        this.$[imsCtxName + '/fsHideUnsupported'] = true;
      }
    }
  }
